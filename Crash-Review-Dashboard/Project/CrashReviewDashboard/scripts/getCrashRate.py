...
"""
covert x.xx.x.xxxxx type build number into x.xx.x (xxxxx) format
"""
...
def get_session(external_id, start_time, end_time, headers, url_prefix, build_version):
    ...
    print(content_1)
    #add logic to assert whether totalSession is the first index in dict_key, if it's first use exist logic , if not use new logic
...
def get_crash(external_id, start_time, end_time, headers, url_prefix, build_version):
    ...
    content_4 = json.loads(crash_request.content.decode('utf-8'))
    total_crash = 0
    content_5 = content_4.get('data').get('project').get('crashlytics')
    #maybe take same actions as session as dict_key index
...
def get_crash_rate_for_country(country):
    ...
    print("===="+country+"====")
    build_version = getCurrentRelease.get_release_build_number_for_and_country(country)
    ...
    if session > 0 and crash > 0:
        ...
        attachment['crash_rate'] = crash_session
    return crash_session,app_version,build_version
